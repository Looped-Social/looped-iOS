            ...
            if let pendingCropImage {
                ProfileImageCropperView(
                    ...
                    onConfirm: { cropped in
                        ...
                        profilePhotoPayload = makeUploadPayload(from: prepared)
                        isShowingImageCropper = false
                        self.pendingCropImage = nil
                    }
                )
...
private extension UserSettingsView {
    ...
    func hydrateFromUser() {
        ...
        initialUsername = normalized(username).lowercased()
        usernameState = .idle
        usernameCheckTask?.cancel()
        firstName = user.firstName ?? ""
        lastName = user.lastName ?? ""
        initialFirstName = normalized(firstName)
        initialLastName = normalized(lastName)
    ...
    var hasUnsavedChanges: Bool {
        if isAnonymousMode {
            return anonDisplayCommunityId != initialAnonDisplayCommunityId
                || anonDisplaySpecialization?.id != initialAnonDisplaySpecializationId
        }

        return normalized(username).lowercased() != initialUsername
            || normalized(firstName) != initialFirstName
            || normalized(lastName) != initialLastName
            || !Calendar.current.isDate(dateOfBirth, inSameDayAs: initialDateOfBirth)
    ...
    func saveProfile(dismissOnSuccess: Bool = false) {
        guard !isSaving else { return }
        if isAnonymousMode {
            saveAnonymousProfile(dismissOnSuccess: dismissOnSuccess)
            return
        }
        ...
        Task {
            ...
            do {
                ...
                profilePhotoPreview = nil
                initialUsername = normalized(username).lowercased()
                initialFirstName = trimmedFirstName
    ...
    func saveAnonymousProfile(dismissOnSuccess: Bool = false) {
        let impact = UIImpactFeedbackGenerator(style: .light)
        impact.impactOccurred()
        saveError = nil
        anonDisplayCommunityError = nil
    ...
    private func handleProfilePhotoSelection(_ newValue: PhotosPickerItem?) async {
        ...
        do {
            ...
            pendingCropImage = image.normalizedOrientation().resized(maxDimension: 1024)
